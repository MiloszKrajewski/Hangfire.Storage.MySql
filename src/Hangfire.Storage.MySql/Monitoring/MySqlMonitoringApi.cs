...
namespace Hangfire.Storage.MySql.Monitoring
{
	internal class MySqlMonitoringApi: IMonitoringApi
	{
		...
		public MySqlMonitoringApi(
			[NotNull] MySqlStorage storage, MySqlStorageOptions storageOptions)
		{
		...
		public StatisticsDto GetStatistics()
		{
			...
			var statistics = UseConnection(
				connection =>
					new StatisticsDto {
						Enqueued = connection.ExecuteScalar<int>(
							jobQuery, new { stateName = "Enqueued" }),
						Failed = connection.ExecuteScalar<int>(
							jobQuery, new { stateName = "Failed" }),
						Processing = connection.ExecuteScalar<int>(
							jobQuery, new { stateName = "Processing" }),
						Scheduled = connection.ExecuteScalar<int>(
							jobQuery, new { stateName = "Scheduled" }),
						Servers = connection.ExecuteScalar<int>(
							$"select count(Id) from `{_storageOptions.TablesPrefix}Server`"),
						Succeeded = connection.ExecuteScalar<int>(
							succeededQuery, new { key = "stats:succeeded" }),
						Deleted = connection.ExecuteScalar<int>(
							succeededQuery, new { key = "stats:deleted" }),
			...
			return UseConnection(
				connection => GetJobs(
					...
					(sqlJob, job, stateData) => new SucceededJobDto {
						...
						SucceededAt =
							JobHelper.DeserializeNullableDateTime(stateData["SucceededAt"])
					}));
		}
			...
			Func<SqlJob, Job, Dictionary<string, string>, TDto> selector)
		{
			...
			foreach (var job in jobs)
			{
				...
				var dto = selector(
					job, DeserializeJob(job.InvocationData, job.Arguments), stateData);

				result.Add(
					new KeyValuePair<string, TDto>(
						job.Id.ToString(), dto));
			}
